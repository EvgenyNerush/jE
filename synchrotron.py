...
n = 10000
subprocess.run(["make", "synchrotron.so"])
main = ctl.load_library("synchrotron.so","./")
...
plt.hist(x,bins=70,density=True,color='b',alpha=0.5,edgecolor='k',label='Metropolis')
#plt.plot(y,bk(y),color='b',label=r'$\frac{dI}{d\Omega}(\omega,\theta = 0)$',linewidth=3,linestyle='--')
plt.legend(loc = 'best',prop={'size': 12})
plt.xlabel(r'$\omega$, MeV/$\hbar$.',fontsize = 12)
plt.ylabel(r'f($\omega$,$\theta = 0$), a.u.',fontsize = 12)
#plt.xlim(0,10)
